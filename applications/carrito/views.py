...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        #logica de carrito de compras cuando usuario esta logueado
...
def remove_cart(request, product_id, cart_item_id):
    ...
    try:  
        #Elimina o disminuye items de usuario en sesion
        if request.user.is_authenticated:
            cart_item = CartItem.objects.get(product = product, user = request.user, id = cart_item_id)
        else:
            #Elimina o disminuye items de usuario fuera de sesion
            cart = Cart.objects.get(cart_id = _cart_id(request))
...
def remove_cart_item(request, product_id, cart_item_id):   
    ...
    #Elimina todo el item del carrito de usuario autenticado
    if request.user.is_authenticated:
        cart_item = CartItem.objects.get(product = product, user = request.user, id = cart_item_id)
    else:
        #Elimina todo el item del carrito de usuario  no autenticado
        cart = Cart.objects.get(cart_id = _cart_id(request))
        cart_item = CartItem.objects.get(product = product, cart = cart, id = cart_item_id)
